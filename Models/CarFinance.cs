...
public class CarFinance
{
    ...
    {
        int? months = Years * 12; decimal? monthlyInterestRate = YearlyInterestRate / 12 / 100; decimal? CarValue = 0; for (int i = 0; i < months; i++)
        {
            CarValue = (CarValue + MonthlyInvestment) * (1 + monthlyInterestRate);
        }
        return CarValue;
    }
}
